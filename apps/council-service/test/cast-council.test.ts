...
import { castCouncil } from '../src/casting/cast-council'
...
describe('castCouncil (spec 05 §API)', () => {
  test('returns N members with seats 0..N-1, scores, diversity, and a vector map', async () => {
    const result = await castCouncil('Should we switch to annual billing?', 4, deps)
  ...
  test('rejects council sizes outside 3–6', async () => {
    await expect(castCouncil('q', 2, deps)).rejects.toThrow()
    await expect(castCouncil('q', 7, deps)).rejects.toThrow()
  })
})
