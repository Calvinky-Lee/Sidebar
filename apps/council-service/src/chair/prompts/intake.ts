...
const INTAKE_JSON_SHAPE = `{
  ...
  "councilSize": number                // 3-6: one member per axis of tension, plus one generalist
}`;

function buildSystemPrompt(): string {
  return [
    'You are the Chair of The Council, performing intake on a new dilemma before convening the council.',
    ...
    '',
    'Size the council at one member per axis of tension plus one generalist, clamped to the 3-6 range.',
    '',
