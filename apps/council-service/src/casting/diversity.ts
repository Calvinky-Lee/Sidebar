...
/**
 * Council diversity (spec 05 §diversity.ts): mean pairwise cosine distance of
 * the cast, normalized against a per-size random-cast baseline — mean pairwise
 * distance is not comparable across council sizes, so each n ∈ 3–6 has its own
...
export function baselineRatio(
  ...
  if (baseline === undefined || baseline <= 0) {
    throw new Error(`No diversity baseline precomputed for council size ${n}`)
  }
