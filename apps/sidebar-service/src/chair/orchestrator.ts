...
export async function runDeliberation(
  ...
  await emit('dilemma_parsed', {
    ...
    sidebarSize: intake.sidebarSize,
  });
  status = transition(status, { type: 'phase_done', phase: 'intake' });

  // --- Casting ---------------------------------------------------------------
  await emit('casting_started', { poolSize: 25, sidebarSize: intake.sidebarSize });
  ...
  const casting = await deps.castingProvider.castSidebar(castingQuery, intake.sidebarSize);
