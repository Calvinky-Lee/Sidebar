 ...
 *
 *   castSidebar(parsedDilemma, size) →
...
export async function castSidebar(
  parsedDilemma: string,
  size: number,
  deps: CastSidebarDeps = {},
): Promise<CastSidebarResult> {
  if (!Number.isInteger(size) || size < 3 || size > 6) {
    throw new Error(`Sidebar size must be an integer in 3–6, got ${size}`)
  }
