...
**Mission:** Own the demo. The deliberation takes 60–90 seconds — your streaming UI is what turns that latency into the show. The show is a **mind headquarters**: the Chair on its platform, four colored blob council members at a console arc, memory orbs of past cases orbiting above, a search bar over past conversations. You touch ONLY the event contract + the REST read endpoints; you must never be blocked on the backend.
...
5. **HQ radial layout + PhaseTracker + ThinkBubbles.** The Chair (verdict-making judge agent) at the **center**, four member seats surrounding it, each with a **fixed-size ThinkBubble** (~30ch × 4 lines, phase-prefixed: 💭 first read → 🗣 opinion → 🔄 reading the others → 🎯 pitch) anchored outward so all four + Chair always fit with zero overlap (spec 07 §ThinkBubble). Plus the six-step tracker. Projector at 1080p is the real demo target.
...
10. **Crystallization + memory-orb field.** On `session_done`, the case condenses into a glowing orb (swirl of the four member hues) that floats up to join the orbiting field. Home screen `OrbField` from `GET /sessions`: hover → dilemma + vote split tooltip; click → replay. Empty state: "no memories yet."
