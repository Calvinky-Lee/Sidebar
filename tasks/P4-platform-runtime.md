...
- **Provide:** the monorepo, `packages/contract`, the sidebar service (hosting P1's runner), SSE infra, tools, Atlas setup, local run scripts, demo mode, metrics capture.
...
1. **Hour 0 — monorepo + `packages/contract`.** pnpm workspace: `apps/web` (Next.js), `apps/sidebar-service` (Hono), `packages/contract` (zod schemas for every SSE event, stance, verdict, persona — §6). Get all three teammates to sign off before anyone writes feature code. Schema parse failures fail loudly everywhere.
2. **Sidebar service scaffold.** Hono, run locally (`pnpm dev` alongside the web app — no cloud hosting in v1). `POST /sessions` → id; `GET /sessions/:id/stream` → SSE; `GET /sessions/:id` + `GET /sessions/:id/events` → replay reads; `GET /sessions?q=` → list/search for the memory-orb field and search bar (Mongo text index, orb payload per contract §REST). Frontend never touches Mongo directly. Bearer token optional locally (off by default; the middleware exists so hosting later is one env var).
...
8. **Local run experience.** One command (`pnpm dev`) brings up web + sidebar service with hot reload; `.env` loading; a `pnpm demo` script that starts everything in demo mode. Working end-to-end skeleton by hour ~12 — integration happens on the actual demo laptop.
